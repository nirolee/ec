...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
	...
	$smarty->assign('brands_new', $brands);
	...
	$smarty->assign('clear_filter',       build_uri('category', array('cid'=>$cat_id), $cat['cat_name'])); // 清空筛选属性
	...
	$smarty->assign('my_comments',      get_comments(50));
	$smarty->assign('on_cat_name',  get_on_cat_name_top($cat_id));	
	$smarty->assign('categories_left',  get_categories_tree_left($cat_id,1)); 
	
	//by guan start
    $smarty->assign('single_show', get_single_show());
	//by guan end
	...
	if($dwt_name == 'category_top')
	{
		...
		$on_top_goods = get_on_top_goods($cat_id,6);
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		/*列表页评分*/
		$get_comment_ranks=$GLOBALS['db']->getOne("SELECT IFNULL(AVG(comment_rank), 0) AS comment_rank FROM " . $GLOBALS['ecs']->table('comment') . " where  comment_type=0 and status =1 and id_value ='$goods_id'");  
		$arr[$row['goods_id']]['comment_ranks']      = ceil($get_comment_ranks) == 0 ? 5 : ceil($get_comment_ranks);
		/*列表页评分*/

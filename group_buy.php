...
function group_buy_list($size, $page)
{
   ...
   $group_buy['num']=$tg['total_order'];
		...
		$group_buy['status'] = $group_buy_info['status'];

		if ($group_buy['price_ladder'][0]['price'] > 0 && $group_buy['shop_price'] > 0) 
		{ 

			$zhekou = round(10 / ( $group_buy['shop_price'] / $group_buy['price_ladder'][0]['price']), 1); 
		} 
		else 
		{ 
			$zhekou = 0; 

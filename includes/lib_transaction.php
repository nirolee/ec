...
function get_user_orders_goods($user_id, $num = 10, $start = 0, $order_id = 0, $is_d = 0)
{
	/* 取得订单列表 */
	$arr    = array();
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		if($row['shipping_status'] == SS_RECEIVED)
		{
			...
			$sql = "SELECT comment_rank FROM " . $GLOBALS['ecs']->table('comment') . " WHERE user_id='$user_id' AND id_value='$row[goods_id]' AND order_id='$row[order_id]'";
		   ...
		   if($comment_rank['comment_rank'])
			{
				if($comment_rank['comment_rank'] < 4)
				{
					continue;
				}
				$row['comment'] = '<a target=_blank style="color:#63C627;" href=goods.php?id=' .$row['goods_id'] . '>已评论</a>';
				$row['is_comm'] = 0;
			}
			else
			{
				$row['comment'] = '<a onclick="dis_comment(' . $i . ');" style="color:blue; cursor:pointer;"' . '>发表评论</a>';
				$row['g_id'] =  'com_' . $i;
				$row['is_comm'] =1;
			}
			...
			if($row['is_single'] == 0)
			{
				$row['single_sun'] = '<a target=_blank style="color:blue;" href=user.php?act=single_sun&order_id=' .$row['order_id'] . '&goods_id=' . $row['goods_id'] . '>发表晒单</a>';
			}
			...
			elseif($row['is_single'] == 2)
			{
				...
				$row['single_sun'] = '<a target=_blank style="color:#63C627;" href=single_sun.php?act=single_show&sid=' .$single['single_id'] . '>已晒单</a>';
			...
			//查询商品标签
			$sql = "SELECT goods_product_tag FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id='$row[goods_id]'";
			$tag = $GLOBALS['db']->getRow($sql);

			$tag_arr = explode('，', $tag['goods_product_tag']);
			$tag = array();
			foreach($tag_arr as $key => $val)
			{
				if(!empty($val))
				{
					$tag[$key]['tag_words'] = $val;
				}
			}

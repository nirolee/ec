    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
			...
			foreach ($brands AS $key => $val)
			{
				...
				$brands[$key]['url'] = build_uri('brand', array('cid' => $row['cat_id'], 'bid' => $val['brand_id']), $row['cat_name']);
...
function get_top10($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.promote_start_date, g.promote_end_date, g.goods_name,g.market_price, g.promote_price, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
           'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g, ' .
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.sales_volume, g.comments_number, g.shop_price AS org_price, g.promote_price, g.sales_volume,' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
 ...
 * @return  array
 */
function get_category_recommend_goods($type = '',$dwt_name='', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='', $num = '')
{
    ...
    {
    	$num = 0;
    	$num = get_library_number($type2lib[$type],$dwt_name);
    }
    else
    {
    	$num = 4;
	...
	foreach($res as $idx=>$row)
	{
		...
		$children = get_children($row['cat_id']);
		 ...
		 $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
			...
			$arr[$idx]['top_goods'] = get_top10($row['cat_id']);
...
{ 
	...
	$sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb,o.add_time , u.user_name ' .
           'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g, ' .
                $GLOBALS['ecs']->table('order_info') . ' AS o, ' .
                $GLOBALS['ecs']->table('order_goods') . ' AS og, ' .
				 $GLOBALS['ecs']->table('users') . ' AS u ' .
           "WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0  AND u.user_id = o.user_id AND og.order_id = o.order_id AND og.goods_id = g.goods_id AND (o.order_status = '" . OS_CONFIRMED .  "' OR o.order_status = '" . OS_SPLITED . "') " .
           "AND (o.pay_status = '" . PS_PAYED . "' OR o.pay_status = '" . PS_PAYING . "') " .
           "AND (o.shipping_status = '" . SS_SHIPPED . "' OR o.shipping_status = '" . SS_RECEIVED . "') " .
	...
	foreach($res as $idx => $row)
	{
		$arr[$idx]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
                                    sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
        ...
        $arr[$idx]['price'] = price_format($row['shop_price']);
...
function browse_related($limit = 25)
{
	if (!empty($_COOKIE['ECS']['history']))
    {
		...
		$sql = 'SELECT g.goods_id,g.cat_id, g.goods_name, g.market_price,g.sales_volume, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img ' .
            "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
		...
		foreach ($res AS $idx => $row)
		{
			...
			$goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
			/*列表页评分*/
		$get_comment_ranks=$GLOBALS['db']->getOne("SELECT IFNULL(AVG(comment_rank), 0) AS comment_rank FROM " . $GLOBALS['ecs']->table('comment') . " where  comment_type=0 and status =1 and id_value ='$row[goods_id]'");  
			$goods[$idx]['comment_ranks']      = ceil($get_comment_ranks) == 0 ? 5 : ceil($get_comment_ranks);
		/*列表页评分*/
		}
...
function get_on_cat_name_top($cat_id)
{
	$sql = 'SELECT cat_name FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
	$cat_name = $GLOBALS['db']->getOne($sql);
	return $cat_name;

...
function brand_get_goods($brand_id, $cate, $size, $page, $sort, $order) {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $arr[$row['goods_id']]['url'] = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
				$arr[$row['goods_id']]['sales_count']      = get_sales_count($row['goods_id']);
    }

    return $arr;
}

function brand_get_goods2($brand_id, $cate, $size, $last, $sort, $order)
{
    $cate_where = ($cate > 0) ? 'AND ' . get_children($cate) : '';
...
/* 商品累计销量带自定义_新增加 开始 by fxzdp.taobao.com */
...
/* 商品累计销量带自定义_新增加 结束 by fxzdp.taobao.com */

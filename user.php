...
if (in_array($action, $ui_arr))
{
	...
	$smarty->assign('group_buy_goods', user_get_group_buy());      // 团购商品
...
//判断是否评论过
	$sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('comment') . " WHERE id_value='$gid' AND order_id='$oid'";
	if($db->getOne($sql) <= 0)
	{
		show_message('商品需要评论后才能晒单哦。', $_LANG['back_page_up'], "user.php?act=single_list", 'info');
		exit;
	}
	...
	$orders = get_user_orders_goods($user_id, 10, 0, $oid, 1);
...
elseif ($action == 'single_sun_insert')
{
	...
	if(($db->getOne($sql)))
	{
		show_message('-_-这个订单商品已经晒单了，重新选择一个吧', $_LANG['back_page_up'], "user.php?act=single_sun", 'error');
	}

	$title_sn = !empty($_POST['title_sn']) ? htmlspecialchars($_POST['title_sn']) : '';
	$desc_sn = !empty($_POST['desc_sn']) ? htmlspecialchars($_POST['desc_sn']) : '';
	$single_img_url = !empty($_POST['single_img_url']) ? $_POST['single_img_url'] : '';
	
	if(empty($single_img_url) || count($single_img_url) < 1 || count($single_img_url) > 10)
	{
		show_message('您还没有选择要晒单的图片哦', $_LANG['back_page_up'], "user.php?act=single_sun&order_id=$order_id&goods_id=$goods_id", 'error');
	}
	...
	'$order_id', '$title_sn', '$desc_sn', 0, '$user_name', 0, '$order_sn', '$time', '$goods_id', '$goods_name', '$user_id', '$order_time', $cat_id)";
	...
	if(!empty($single_id))
	{
		...
		foreach($single_img_url as $k => $v)
		{
			...
			$img_path = $dir.'/'.$sub_dir.'/single_img/'.trim($img_name).trim($img_ext);
			...
			if(move_image_file_single(ROOT_PATH.trim($v), ROOT_PATH.$dir.'/'.$sub_dir.'/single_img/'.trim($img_name).trim($img_ext)))
			{
 ...
 * 评论购买完成的商品
 */
elseif($action == 'comm_order_goods')
{
	...
	$goods_id = empty($_POST['id']) ? '' : $_POST['id'];
	$goods_tag = empty($_POST['goods_tag_box']) ? '' : $_POST['goods_tag_box'];
	$tag = implode(',', $goods_tag);
	$addtime = gmtime();
	$ip = real_ip();
	
	$sql = "INSERT INTO " . $ecs->table('comment') . "(comment_type, id_value, email, user_name, content, comment_rank, add_time, ip_address
			, status, parent_id, user_id, single_id, order_id, goods_tag)VALUES('0', '$goods_id', ' $_SESSION[email]', '$_SESSION[user_name]', '$content', '$rank',
	'$addtime', '$ip', '0', '0', '$_SESSION[user_id]', '0', '$order_id', '$tag')";
	if($db->query($sql))
	{
		show_message('留言成功，请等待管理员审核', $_LANG['back_page_up'], "user.php?act=single_list", 'success');
 ...
 * @return  array
 */
function user_get_group_buy()
{
    $time = gmtime();
    $limit = get_library_number('group_buy', 'user');
